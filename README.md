...
*(Every spin prints a 3x3 grid of symbols, tallies up matching rows, and updates your balance live in the terminal. Prompts, results, and the grid itself are color-coded in the real terminal via `colorama`.)*
...
| 🖨️ **Colored Grid Output** | Renders the spin result as a `3x3` grid separated by `|`, colored via `colorama` |
| 🧹 **Clean Start** | Clears the terminal screen at the start of every run |
...
├── main.py          # Entry point — imports and runs game()
├── game.py          # All game logic (deposit, betting, spin, win checking)
├── constant.py       # Game constants (limits, grid size, symbol odds/payouts)
├── utils.py          # Small helpers (clear_screen)
...
### `main.py`
Just wires things up — imports `game()` from `game.py` and calls it. No game logic lives here.

### `game.py`

**`game()`**
- Clears the terminal (`utils.clear_screen()`).
...
- Calls `check_winnings()` to calculate the payout and returns the **net change** in balance (`winnings - total_bet`) back to `game()`.

**`get_slot_machine_spin(rows, cols, symbol)`**
...
- Prints each symbol in light blue via `colorama`.
...
### `constant.py`
Holds every tunable value in one place: `MAX_LINES`, `MIN_BET`, `MAX_BET`, `ROWS`, `COLS`, `symbols_count`, and `symbols_value`.
...
- ✅ Splitting a single script into modules (constants, utilities, and game logic)
- ✅ Using a third-party library (`colorama`) for terminal styling
